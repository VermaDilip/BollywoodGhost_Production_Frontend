...
import { getTracksByType } from "../services/servicesApi"; // Replace with actual path
import AiCoverShimmer from "../components/shimmer/AiCoverShimmer";
import { FiChevronLeft, FiChevronRight } from "react-icons/fi";
import "./AiCover.css"; // Optional: for custom styles
...
export default function AiCover() {
  ...
  // Load AI cover tracks (trackType = 2)
  useEffect(() => {
    setLoading(true);
    getTracksByType(2)
      .then((data) => setVideos(data))
      .catch((err) => console.error("Error fetching AI Covers:", err))
  ...
  return (
    <>
      ...
      {loading ? (
        <AiCoverShimmer count={visibleCount} />
      ) : (
        <div className="relative">
          ...
          <div
            ref={containerRef}
            className="scrollbar-hide flex overflow-x-auto gap-4 sm:gap-2 px-4 sm:px-6 pb-6 scroll-smooth"
